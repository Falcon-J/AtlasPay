...
docker compose exec -T inventory-service wget --no-verbose --tries=1 --spider http://localhost:8082/health/ready | Out-Null
if ($LASTEXITCODE -ne 0) {
    throw "Inventory service readiness check failed"
}
